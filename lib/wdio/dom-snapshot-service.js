...
import { DOMSnapshotBrowser } from '../node/domSnapshot.js'
...
export class DOMSnapshotService {
  ...
  async _executeInBrowser (browser, selector, method) {
    ...
    return browser.execute(function (sel, opts, meth, snapshotClass) {
    // Parentheses prevent ASI issue with leading newline
    ...
    }, selector, browserOpts, method, DOMSnapshotBrowser)
  }
  ...
  _saveSnapshot (name, type, result) {
    const snapshot = {
      name,
      type,
      timestamp: new Date().toISOString(),
      hash: this.hash(result.data),
      meta: result.meta,
  ...
  async _compareSnapshot (browser, name, type, selector, method) {
    ...
    // Capture actual
    const result = await this._executeInBrowser(browser, selector, method)
    const actual = this._saveSnapshot(name, type, result)

    // Load baseline
    const baseline = JSON.parse(fs.readFileSync(baselinePath, 'utf8'))

    // Compare hashes
    const match = baseline.hash === actual.hash

    // Generate diff if mismatch
    let diff = null
    if (!match) {
      diff = this.generateDiff(baseline.data, actual.data)
    ...
    return {
      ...
      actual: { hash: actual.hash, timestamp: actual.timestamp },
